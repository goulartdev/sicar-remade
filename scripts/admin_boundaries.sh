(
  ...
  tippecanoe -z5 -o "$uf_output.pmtiles" --hilbert --include "NM_UF" -l "admin_boundaries" "$uf_output.geojson"
  ...
  tippecanoe -zg -Z6 -o "$mun_output.pmtiles" --hilbert --include "NM_MUN" -l "admin_boundaries" "$mun_output.geojson"
  rm "$mun_output.geojson"

  tile-join -l "admin_boundaries" -o "$output_path/admin_boundaries.pmtiles" "$mun_output.pmtiles" "$uf_output.pmtiles"
)
